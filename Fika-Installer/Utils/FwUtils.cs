...
public static class FwUtils
{
    ...
    public static bool FirewallRuleExists(string displayName, string program, out bool ruleExists)
    {
        ...
        var psProcess = ProcUtils.Execute("powershell.exe", $"{_powershellCmd} \"{firewallCmd}\"", ProcessWindowStyle.Hidden);
    ...
    {
        var firewallRuleSet = FirewallRules(installDir);

        var createFirewallRulesRequired = false;

        Logger.Log("Checking existing firewall rules...");

        foreach (var rule in firewallRuleSet)
        {
            var success = FirewallRuleExists(rule.DisplayName, rule.Program, out var ruleExists);
        ...
        if (createFirewallRulesRequired || force)
        {
            ...
            else
            {
                try
                {
                    var elevatedProcess = ProcUtils.Execute(Application.ExecutablePath, $"create-firewall-rules {installDir}", ProcessWindowStyle.Minimized, true);
    ...
    public static bool RemoveFirewallRules(string installDir, bool force = false)
    {
        ...
        var removeFirewallRulesRequired = false;
        ...
        if (removeFirewallRulesRequired || force)
        {
            ...
            else
            {
                try
                {
                    var elevatedProcess = ProcUtils.Execute(Application.ExecutablePath, $"remove-firewall-rules {installDir}", ProcessWindowStyle.Minimized, true);
    ...
    public static void CreateFirewallRulesElevated(string installDir)
    {
        ...
        foreach (var rule in FirewallRules(installDir))
        {
