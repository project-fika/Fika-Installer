...
public sealed class SptInstaller(string sptDir)
{
    public bool InstallSpt(string installDir, InstallMethod installType, bool headless = false)
    {
        ...
        if (installType == InstallMethod.Symlink)
        {
            ...
            var escapeFromTarkovDataPath = Path.Combine(sptDir, eftDataFolderName);
            var escapeFromTarkovDataFikaPath = Path.Combine(installDir, eftDataFolderName);
        ...
        if (headless)
        {
            var sptPatchesDir = Path.Combine(sptDir, @"SPT_Runtime\SPT_Data\Launcher\Patches");
            var sptPatchesDirDest = Path.Combine(installDir, @"SPT_Runtime\SPT_Data\Launcher\Patches");
    ...
    {
        var sptPatchesDir = Path.Combine(installDir, @"SPT_Runtime\SPT_Data\Launcher\Patches");

        try
        {
            foreach (var patchPath in Directory.GetDirectories(sptPatchesDir))
            {
                DirectoryInfo patchPathDirectoryInfo = new(patchPath);

                // get all patch files within patchpath and it's sub directories.
                foreach (var file in patchPathDirectoryInfo.GetFiles("*.delta", SearchOption.AllDirectories))
                {
                    FileInfo target;
    ...
    public bool GenerateSptShortcuts(string installDir)
    {
        try
        {
            var sptFolder = Path.Combine(installDir, "SPT");

            var sptLauncherPath = Path.Combine(sptFolder, "SPT.Launcher.exe");

            if (File.Exists(sptLauncherPath))
            {
                var sptLauncherShortcutPath = Path.Combine(installDir, "SPT.Launcher.lnk");
            ...
            var sptServerPath = Path.Combine(sptFolder, "SPT.Server.exe");

            if (File.Exists(sptServerPath))
            {
                var sptServerShortcutPath = Path.Combine(installDir, "SPT.Server.lnk");
